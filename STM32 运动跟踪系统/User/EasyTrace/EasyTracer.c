...
//============================================================================
extern RESULT Resu;    
static int SearchCentre_1(unsigned int *x,unsigned int *y,const TARGET_CONDI *Condition,const SEARCH_AREA *Area)
{
	unsigned int SpaceX,SpaceY,i,j,k,FailCount=0;
	COLOR_RGB Rgb;
	COLOR_HSL Hsl;
	...
	for(i=Area->Y_Start;i<Area->Y_End;i+=SpaceY)
	{
		for(j=Area->X_Start;j<Area->X_End;j+=SpaceX)
		{
			...
			if(k == SpaceX+SpaceY)
			{
				*x = j+SpaceX/2;
				*y = i+SpaceY/2;
				if( *x <= Resu.x-Resu.w/2 || *y <= Resu.y-Resu.h/2 
				|| *x >= Resu.x+Resu.w/2 || *y >= Resu.y+Resu.h/2 )
						return 1;
				else
						 break;
			}
...
int Trace(const TARGET_CONDI *Condition,RESULT *Resu)
{
	...
	if(flag_ == 0)
	{
		...
		{ //这里重新赋值的原因是在上一次搜索成功后 改变了区域 ，所以这次遍历需要重新赋值整个区域
			Area.X_Start= IMG_X	;
...
int Trace_1(const TARGET_CONDI *Condition,RESULT *Resu)
{
	...
	if(flag_ == 0)
	{
		if(SearchCentre_1(&x0,&y0,Condition,&Area))   //搜索腐蚀中心
			flag_ = 1;
		else
		{
			Area.X_Start= IMG_X	;
			...
			if(SearchCentre_1(&x0,&y0,Condition,&Area))	
			{
				flag_ =0;
				return 0;
			}	
	...
	for(i=0;i<ITERATE_NUM;i++)
		Corrode(Result.x,Result.y,Condition,&Result);   //从腐蚀中心向外腐蚀，得到新的腐蚀中心
	if( Result.x <= Resu->x-Resu->w/2 || Result.y <= Resu->y-Resu->h/2 
		||Result.x >= Resu->x+Resu->w/2 || Result.y >= Resu->y+Resu->h/2 )
	{
	if(Corrode(Result.x,Result.y,Condition,&Result))
	{
		x0      = Result.x;
